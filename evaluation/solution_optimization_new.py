...
def optimize(var1, var2, gradient_computor, optimizer, test_time_objective):
    evaluation_1 = test_time_objective(var1)
    evaluation_2 = test_time_objective(var2)

    gradient = gradient_computor(tg.Variable(
        GRADIENT_TEMPLATE.format(answer1=var1.value, evaluation1=evaluation_1.value,
                                 answer2=var2.value, evaluation2=evaluation_2.value),
        role_description="gradient computation"))

    optimizer_response = optimizer(tg.Variable(
        OPTIMIZATION_TEMPLATE.format(answer1=var1.value, evaluation1=evaluation_1.value,
                                     answer2=var2.value, evaluation2=evaluation_2.value,
                                     gradient=gradient.value), role_description="optimizer response"))

    resp = extract_answer(optimizer_response.value)
    ...
    var1 = tg.Variable(var2.value, requires_grad=False, role_description=var2.role_description)
    var2 = tg.Variable(new_answer, requires_grad=False, role_description=var2.role_description)
...
def run_test_time_training(sample):
    ...
    for _ in range(args.max_iterations):
        instance_var1, instance_var2 = optimize(instance_var1, instance_var2, gradient_computor, optimizer, test_time_objective)
